/* Design D — Classified Dossier. Same real content as the main site, restyled. */
...
function fileBody(item) {
  if (item.groups) {
    return `<ul class="file-groups">${item.groups.map(g => `
      <li>
        <p class="file-group-label">${g.label}</p>
        <ul class="file-bullets">${g.items.map(i => `<li>${i}</li>`).join("")}</ul>
      </li>`).join("")}</ul>`;
  }
  if (item.bullets) {
    return `<ul class="file-bullets">${item.bullets.map(i => `<li>${i}</li>`).join("")}</ul>`;
  }
  return `<p class="file-desc">${item.desc}</p>`;
}

function renderFiles(containerId, items) {
  const el = document.getElementById(containerId);
  el.innerHTML = items.map((item, i) => `
    <article class="file-card reveal">
      <span class="file-card-tab">FILE ${String(i + 1).padStart(2, "0")} · ${item.status}</span>
      <div class="file-head">
        <p class="file-title">${item.title}</p>
        <span class="file-date">${item.date}</span>
      </div>
      <p class="file-org">${item.org}</p>
      ${fileBody(item)}
      <ul class="file-tags">${item.tags.map(t => `<li>${t}</li>`).join("")}</ul>
    </article>
  `).join("");
}
...
renderFiles("educationList", EDUCATION);
renderCertifications("certStrip", CERTIFICATIONS);
renderFiles("experienceList", EXPERIENCE);
